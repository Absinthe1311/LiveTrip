...
export default function OptimizedDayMap({
  ...
  const [infoWindowPosition, setInfoWindowPosition] = useState<{ x: number; y: number } | null>(
    null
  );
  ...
  const handleMarkerClick = (item: any, position: number[]) => {
    ...
    if (mapRef.current && position.length >= 2) {
      const pixel = mapRef.current.lnglatTocontainer(
        (window as any).AMap.LngLat(position[0], position[1])
      );
  ...
  useEffect(() => {
    ...
    AMapLoader.load({
      ...
      .then((AMap) => {
        ...
        // 计算中心点
        const centerLng =
          coordinates.reduce((sum, coords) => sum + coords[0], 0) / coordinates.length;
        const centerLat =
          coordinates.reduce((sum, coords) => sum + coords[1], 0) / coordinates.length;
        ...
        const map =
          mapRef.current ||
          new AMap.Map(mapContainer.current, {
        ...
        if (
          hotel?.location &&
          hotelRecommendations &&
          !hotelRecommendations.some((h: Hotel) => h.name === hotel.name)
        ) {
          const hotelCoords = hotel.location.split(',').map(Number);
        ...
        if (showAllRestaurants && restaurantRecommendations) {
          ...
          if (
            restaurant?.location &&
            !restaurantRecommendations.some((r: Restaurant) => r.name === restaurant.name)
          ) {
