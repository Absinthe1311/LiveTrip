...
export default function TodayGlass() {
    ...
    const loadCitySpots = async () => {
      try {
        const response = await apiClient.get(
          `/spots/city/${encodeURIComponent(currentTrip.destination)}`,
          {
            params: { limit: 500 },
          }
        );
  ...
  const calculateBudget = () => {
    ...
    const spent =
      currentTrip.budget.tickets +
      currentTrip.budget.food +
      currentTrip.budget.accommodation +
      currentTrip.budget.transportation +
      currentTrip.budget.shopping +
      currentTrip.budget.other;
    return {
      spent,
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <div className="text-xs text-white/60">
                        {formatDateShort(currentTrip.startDate).month}月
                        {formatDateShort(currentTrip.startDate).day}日 -{' '}
                        {formatDateShort(currentTrip.endDate).month}月
                        {formatDateShort(currentTrip.endDate).day}日
                      </div>
                    </div>
                  </div>
                  <ChevronDown
                    className={`h-5 w-5 transition-transform ${showTripSelector ? 'rotate-180' : ''}`}
                  />
                </button>
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-500">
                            {formatDateShort(trip.startDate).month}月
                            {formatDateShort(trip.startDate).day}日 -{' '}
                            {formatDateShort(trip.endDate).month}月
                            {formatDateShort(trip.endDate).day}日
                          </div>
                        </div>
                  ...
                  return (
                    ...
                    >
                      <div className="text-center">
                        <div
                          className={`text-2xl font-bold ${isSelected ? 'text-white' : 'text-white/90'}`}
                        >
                          {dateInfo.day}
                        </div>
                        <div
                          className={`text-xs ${isSelected ? 'text-white/90' : 'text-white/60'}`}
                        >
                          周{dateInfo.weekday}
                        </div>
                      </div>
                      ...
                      return (
                        <div key={item.id} className="flex items-start gap-3">
                          ...
                          <div className="flex flex-col items-center flex-shrink-0">
                            ...
                            {!isLast && (
                              <div
                                className={`w-0.5 h-16 flex-shrink-0 ${
                                  isVisited
                                    ? 'bg-gray-400/40'
                                    : isInProgress
                          ...
                          >
                            <div
                              className={`bg-white/60 backdrop-blur-xl rounded-xl px-4 py-2 border transition-all duration-300 mb-2 ${
                                isSelected
                            ...
                            <div
                              className={`text-sm ${isVisited ? 'text-white/40 line-through' : 'text-white/80'} truncate`}
                            >
                              {item.name}
                            </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="font-semibold text-gray-800">
                              {formatTime(selectedAttraction.startTime)} -{' '}
                              {formatTime(selectedAttraction.endTime)}
                            </div>
                          </div>
                        </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  ...
                                  <div className="text-sm font-semibold text-gray-800">
                                    {iotData[selectedAttraction.spotId].crowdLevel === 1
                                      ? '宽松'
                                      : iotData[selectedAttraction.spotId].crowdLevel === 2
                                        ? '适中'
                                        : iotData[selectedAttraction.spotId].crowdLevel === 3
                                          ? '拥挤'
                                          : '非常拥挤'}
                                  </div>
                                </div>
                ...
                packingItems.map((item) => (
                  ...
                  >
                    ...
                    {editingItem?.id === item.id ? (
                      ...
                      <div className="flex-1">
                        <span
                          className={item.isPacked ? 'text-gray-400 line-through' : 'text-gray-800'}
                        >
                          {item.itemName}
                        </span>
        ...
        >
          <div className="space-y-4">
            <div>
              ...
              <Input value={currentTrip?.totalBudget || 0} disabled prefix="¥" />
            </div>
            <div>
