...
export async function getSpotWeatherWithMicroclimate(
  ...
  try {
    ...
    if (!spot) {
      ...
      const defaultAdjustment = applyMicroclimateAdjustment(
        baseWeather,
        MicroclimateType.DEFAULT
      );
      return {
    ...
    console.log(`   降雨概率: ${baseWeather.rainProbability}% → ${adjustedWeather.rainProbability}%`);
 ...
 */
export async function batchIdentifyMicroclimateTypes(
  spotIds: string[]
): Promise<Map<string, {
  microclimateType: MicroclimateType;
