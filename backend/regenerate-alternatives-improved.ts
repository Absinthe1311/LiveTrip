...
async function regenerateAlternativesImproved() {
  try {
    console.log('='.repeat(80));
    console.log('批量重新生成备选关系（改进版）');
    console.log('='.repeat(80));
    console.log('优化目标：');
    console.log('  1. 每个景点最多3个备选，最少1个');
    console.log('  2. 一个景点只能出现在一个备选列表中（不重复）');
    ...
    // 步骤3：为每个景点生成备选关系（改进算法）
    console.log('\n📝 步骤3：为每个景点生成备选关系...');
    console.log('策略：每个景点生成1-3个备选，确保不重复使用\n');
    ...
    for (const [city, citySpots] of spotsByCity.entries()) {
      ...
      // 关键改进：记录已被用作备选的景点（全局不重复）
      const usedAsAlternative = new Set<string>();
      ...
      for (let i = 0; i < citySpots.length; i++) {
        ...
        try {
          // 获取候选景点（同城市其他景点，且未被用作备选）
          const candidates = citySpots.filter(s => 
            s.id !== spot.id && !usedAsAlternative.has(s.id)
          );

          if (candidates.length === 0) {
            console.log(`  ⚠️  没有可用候选景点（所有景点已被用作备选），跳过`);
            failCount++;
            continue;
          }
          ...
          for (const candidate of sortedCandidates) {
            ...
            // 检查该候选景点是否已被用作备选
            if (!usedAsAlternative.has(candidate.id)) {
              alternatives.push({
                id: candidate.id,
                priority: alternatives.length + 1,
              });
              // 标记为已使用
              usedAsAlternative.add(candidate.id);
            }
          }
          ...
          if ((i + 1) % 10 === 0) {
            ...
            console.log(`  📊 已用作备选的景点：${usedAsAlternative.size} 个`);
          }
      ...
      console.log(`  已用作备选的景点：${usedAsAlternative.size} 个（不重复）`);
    }
    ...
    const duplicateUsage = alternativeUsage.filter(item => item._count.id > 1);
    if (duplicateUsage.length === 0) {
      console.log(`  ✅ 验证通过：没有景点重复出现在多个备选中！`);
    } else {
      console.log(`  ❌ 发现 ${duplicateUsage.length} 个景点重复出现在多个备选中：`);
      for (const item of duplicateUsage.slice(0, 5)) {
        const spot = await prisma.spot.findUnique({
          where: { id: item.alternativeSpotId },
          select: { name: true, city: true },
        });
        console.log(`    ${spot?.name} (${spot?.city}): 被使用 ${item._count.id} 次`);
      }
    }
...
// 运行
regenerateAlternativesImproved()
  .then(() => {
    console.log('\n✅ 脚本执行完成');
    process.exit(0);
  })
